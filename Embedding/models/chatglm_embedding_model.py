...
class ChatGLMEmbeddingModel(nn.Module):
    ...
    def __init__(self, llm_model_name_or_path: str, max_seq_len: int = 512, adapter_path=None, device='cuda',
                 **kwargs):
        super().__init__()
        ...
        self.embedding_layer = GLMBlock(self.chatglm.config, 1, device=self.device)
        if adapter_path:
    ...
    def forward(self, inputs):
        ...
        outputs, _ = self.embedding_layer(hidden_states)
        outputs = outputs.transpose(0, 1)  # [Batch, Seq_len, hidden_size]
